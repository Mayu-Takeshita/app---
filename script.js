document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderMemos = () => {
        ...
        sortedSubjects.forEach(subject => {
            ...
            groupedMemos[subject].forEach(memo => {
                ...
                listItem.innerHTML = `<div class="memo-header"><strong>${memo.subject}</strong><span>${memo.date}</span></div><div class="memo-body">${marked.parse(memo.memo)}</div>${memo.image ? `<img src="${memo.image}" alt="添付画像" class="memo-image">` : ''}<div class="memo-actions"><button class="edit-btn">編集</button><button class="delete-btn">削除</button></div>`;
    ...
    memoForm.addEventListener('submit', e => {
        ...
        const memos = loadMemos();
        const now = new Date();
        memos.push({ id: Date.now(), subject, memo, image: currentImageBase64, date: `${now.getFullYear()}/${String(now.getMonth() + 1).padStart(2, '0')}/${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}` });
    ...
    memoImageInput.addEventListener('change', e => {
        const file = e.target.files[0]; if (!file) { resetImageInput(); return; }
