document.addEventListener('DOMContentLoaded', () => {
    ...
    const subjectsFromTimetable = new Set(Object.values(timetableData).flatMap(day => Object.values(day)));
    const allSubjects = ["（手動で入力する）", ...Array.from(subjectsFromTimetable).sort(), "その他"];
    ...
    const renderMemos = () => {
        ...
        const groupedMemos = filteredMemos.reduce((acc, memo) => {
            (acc[memo.subject] = acc[memo.subject] || []).push(memo);
            return acc;
        }, {});
        
        const sortedSubjects = Object.keys(groupedMemos).sort((a, b) => {
            if (sortOrder.includes('subject')) {
                return sortOrder === 'subject-asc' ? a.localeCompare(b) : b.localeCompare(a);
            }
        ...
        sortedSubjects.forEach(subject => {
            ...
            groupedMemos[subject].forEach(memo => {
                ...
                listItem.innerHTML = `
                    <div class="memo-header"><strong>${memo.subject}</strong><span>${memo.date}</span></div>
                    <div class="memo-body">${marked.parse(memo.memo)}</div>
                    ${memo.image ? `<img src="${memo.image}" alt="添付画像" class="memo-image">` : ''}
                    <div class="memo-actions"><button class="edit-btn">編集</button><button class="delete-btn">削除</button></div>`;
    ...
    timetableGrid.addEventListener('click', e => {
        if (e.target.classList.contains('filled')) {
            switchPage(memoPage);
            setTimeout(() => {
                searchBox.value = e.target.dataset.subject;
                renderMemos();
                window.scrollTo(0, 0);
            }, 50);
        }
    ...
    memoForm.addEventListener('submit', e => {
        ...
        if (!subject || !memo) {
            alert('科目とメモの両方を入力してください。');
            return;
        }
        const memos = loadMemos();
        const now = new Date();
        memos.push({
            id: Date.now(), subject, memo, image: currentImageBase64,
    ...
    memoImageInput.addEventListener('change', e => {
        ...
        if (!file) { resetImageInput(); return; }
        const reader = new FileReader();
        reader.onload = e => { currentImageBase64 = e.target.result; imagePreview.src = currentImageBase64; imagePreviewContainer.classList.remove('hidden'); };
    ...
    exportBtn.addEventListener('click', () => {
        const memos = loadMemos();
        if (memos.length === 0) { alert('エクスポートするデータがありません。'); return; }
        ...
        reader.onload = e => {
            try {
                ...
                if (Array.isArray(importedMemos) && confirm(`${importedMemos.length}件のデータを追加しますか？`)) {
                    ...
                    saveMemos([...currentMemos, ...newMemos]);
                    renderMemos();
